---
name: gatekeep-opportunity-auditor
description: Focus on fact-checking, safety policies, and final validation for: Final gatekeeper above all agents: decide if a personality is worth covering, estimate whether it can generate 50+ stories, identify which audiences/countries care, assess YouTube saturation, shareability and retention likelihood. Use when the Story Opportunity Intelligence Auditor agent must act on its inputs and produce its defined output.
metadata:
  agent: story-opportunity-intelligence-auditor
  source: Project Atlas Requirements §3.7
  layer: Executive
  host: vps
---

# gatekeep-opportunity-auditor
...
## When to use
- When the Story Opportunity Intelligence Auditor agent is invoked in the Executive pipeline and its inputs are available.
